# AI Swing Trading SaaS (NSE)

Production-grade scaffold for a hedge-fund-style AI swing trading platform. It covers daily heavy ML signals (Kronos/StockFormer/TFT/CatBoost), Smart Money Concepts validation, hourly 1H/4H entry refinement only, and Elite BUY-only auto execution with PPO-based exits (stubbed hooks).
...
- **Daily** (`POST /pipeline/run-daily`):
  ...
  4) Stub Kronos/StockFormer/TFT/CatBoost fusion -> BUY/SELL/HOLD signal with entry/SL/TP/SMC flags
...
- pk-stock-screener is used strictly for universe reduction; SMC features are validation/feature inputs, not execution logic.
...
- Model registry (`backend/app/services/model_registry.py`) enforces mode guardrails: `MODE=stub` uses deterministic stubs, `MODE=prod` requires artifact paths for Kronos/StockFormer/TFT/CatBoost and will fail if missing or not implemented (no heuristics in prod).
- Kronos download helper: `PYTHONPATH=.. python backend/scripts/download_kronos.py --dest artifacts/kronos` (uses Hugging Face repo `NeoQuasar/Kronos-base`; set `HF_TOKEN` if gated). Update `kronos_artifact_path` in `.env` to point to the downloaded directory.
- No secrets or per-user inference in code; signals generated centrally and tier-gated at fetch.
